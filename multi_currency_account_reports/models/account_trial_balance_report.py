...
class TrialBalanceCustomHandler(models.AbstractModel):
    ...
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals):
        def _update_column(line, column_key, new_value, blank_if_zero=True, currency=False):
            currency = currency or self.env.company.currency_id
            line['columns'][column_key]['name'] = self.env['account.report'].format_value(new_value, currency=currency, figure_type='monetary', blank_if_zero=blank_if_zero)
            line['columns'][column_key]['no_format'] = new_value

        def _update_balance_columns(line, debit_column_key, credit_column_key, debit2_column_key, credit2_column_key, total_diff_values_key):
            debit_value = line['columns'][debit_column_key]['no_format']
            credit_value = line['columns'][credit_column_key]['no_format']
            debit2_value = line['columns'][debit2_column_key]['no_format']
            credit2_value = line['columns'][credit2_column_key]['no_format']

            if debit_value and credit_value and debit2_value and credit2_value:
                new_debit_value = 0.0
                new_credit_value = 0.0
                new_debit2_value = 0.0
                new_credit2_value = 0.0
                ...
                _update_column(line, debit2_column_key, new_debit2_value, currency=self.env.company.currency_id2)
                _update_column(line, credit2_column_key, new_credit2_value, currency=self.env.company.currency_id2)
        ...
        for line in lines[:-1]:
            ...
            if res_model == 'account.account':
                _update_balance_columns(line, 0, 1, 2, 3, 'initial_balance')

                # End balance
                _update_balance_columns(line, -4, -3, -2, -1, 'end_balance')
    ...
    def _caret_options_initializer(self):
        return {
            'trial_balance': [
                {'name': _("General Ledger"), 'action': 'caret_option_open_general_ledger_multi_currency'},
