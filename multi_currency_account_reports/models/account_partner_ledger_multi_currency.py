...
class PartnerLedgerMultiCurrencyCustomHandler(models.AbstractModel):
    _name = 'account.partner.ledger.multi.currency.report.handler'
    _inherit = 'account.report.custom.handler'
    _description = 'Partner Ledger Custom Handler MultiCurrency'
    ...
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals):
        ...
        totals_by_column_group = {
            column_group_key: {
                total: 0.0
                for total in ['debit', 'credit', 'debit2', 'credit2', 'balance']
            }
        ...
        for partner, results in self._query_partners(options):
            ...
            for column_group_key in options['column_groups']:
                ...
                partner_values[column_group_key]['debit2'] = partner_sum.get('debit2', 0.0)
                partner_values[column_group_key]['credit2'] = partner_sum.get('credit2', 0.0)
                ...
                totals_by_column_group[column_group_key]['debit2'] += partner_values[column_group_key]['debit2']
                totals_by_column_group[column_group_key]['credit2'] += partner_values[column_group_key]['credit2']
    ...
    def _custom_options_initializer(self, report, options, previous_options=None):
        ...
        if exch_code:
            domain += ['!', '&', '&', '&', '&', '&', ('credit', '=', 0.0), ('credit2', '=', 0.0), ('debit', '=', 0.0), ('debit2', '=', 0.0), ('amount_currency', '!=', 0.0), ('journal_id', 'in', exch_code.ids)]
    ...
    def _query_partners(self, options):
        ...
        """
        def assign_sum(row):
            fields_to_assign = ['balance', 'debit', 'credit', 'debit2', 'credit2']
        ...
        self._cr.execute(query, params)
        totals = {}
        for total_field in ['debit', 'credit', 'debit2', 'credit2', 'balance']:
        ...
        for row in self._cr.dictfetchall():
            ...
            totals['debit2'][row['column_group_key']] += row['debit2']
            totals['credit2'][row['column_group_key']] += row['credit2']
        ...
        if None in groupby_partners:
            ...
            for column_group_key in options['column_groups']:
                ...
                groupby_partners[None][column_group_key]['debit2'] += totals['credit2'][column_group_key]
                groupby_partners[None][column_group_key]['credit2'] += totals['debit2'][column_group_key]
                ...
                if None in rslt:
                    rslt[None].append({
                        **aml_result,
                        ...
                        'debit2': aml_result['credit2'],
                        'credit2': aml_result['debit2'],
    ...
    def _get_report_line_partners(self, options, partner, partner_values):
        ...
        for column in options['columns']:
            ...
            if col_expr_label in {'debit', 'credit', 'balance'}:
                ...
                formatted_value = report.format_value(value, currency=self.env.company.currency_id2, figure_type=column['figure_type'], blank_if_zero=column['blank_if_zero'])
            else:
                formatted_value = report.format_value(value, figure_type=column['figure_type']) if value is not None else value

            unfoldable = unfoldable or (col_expr_label in ('debit', 'credit', 'debit2', 'credit2') and not company_currency.is_zero(value))
        ...
        for column in options['columns']:
            ...
            if col_value is None:
                ...
                if col_expr_label == 'date_maturity':
                    ...
                    currency = self.env.company.currency_id2
                    formatted_value = report.format_value(col_value, currency=currency, figure_type=column['figure_type'])
    ...
    def _get_report_line_total(self, options, totals_by_column_group):
        ...
        for column in options['columns']:
            ...
            if col_expr_label in {'debit', 'credit', 'balance'}:
                ...
                formatted_value = report.format_value(value, currency=self.env.company.currency_id2,
