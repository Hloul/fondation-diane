...
class AccountMoveLine(models.Model):
    ...
    company_currency_id2 = fields.Many2one(string='Second Company Currency', related='company_id.currency_id2',
                                          readonly=True)
    computed_rate = fields.Float(string='Computed Rate', compute='_compute_conversion_rate', inverse='_inverse_conversion_rate', store=True, copy=False)
    ...
    def _inverse_conversion_rate(self):
        for rec in self:
          ...
          _logger.info('MC inverse: computed_rate %s', rec.computed_rate)
